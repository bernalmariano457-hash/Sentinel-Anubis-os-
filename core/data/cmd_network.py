...
class NetworkCommands(_DomainBase):
    ...
    def radar(self):
        ...
        if _tiene_display and self._modulo_ok("geomap"):
            try:
                s.geomap.abrir_mapa()
            except Exception as e:
                self.console.print(
                    f"[yellow][!] Mapa web no disponible: {e}[/yellow]")
        elif not _tiene_display:
            self.console.print(
                "[dim][Radar] Sin DISPLAY — modo solo terminal (sin mapa web)[/dim]")
        ...
        try:
            while True:
                ...
                # Actualizar mapa solo si hay display y geomap cargado
                if _tiene_display and getattr(s, "geomap", None):
                    try:
