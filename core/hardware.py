...
def check_usb_permissions() -> tuple[bool, str]:
    ...
    if not os.access(str(usb_path), os.R_OK):
        ...
        return False, (
            ...
            f"Luego cierra sesión y vuelve a entrar, o: newgrp plugdev\n"
    ...
    try:
        ...
        if plugdev.gr_gid not in os.getgroups():
            return False, (
...
def check_rtlsdr_rules() -> tuple[bool, str]:
    ...
    if any(p.exists() for p in rules_paths):
        return True, ""
...
class SDRManager:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            pass  # defaults del __init__ se mantienen
    ...
    def _try_rtlsdr(self) -> bool:
        try:
            ...
            sdr.gain = "auto" if self._cfg.gain == "auto" else float(
                self._cfg.gain)

            if getattr(self._cfg, "bias_tee", False) and hasattr(sdr, "set_bias_tee"):
            ...
            log.info(
                f"RTL-SDR conectado — {self._hw_info.get('serial', 'desconocido')} "
                f"| PPM: {self._cfg.ppm_correction} | Gain: {self._cfg.gain} dB"
            )
            return True
