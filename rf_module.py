"""
╔══════════════════════════════════════════════════════════════════╗
║  APEX SENTINEL — rf_module.py                                    ║
║  Fachada unificada del módulo RF (hardware, mock, DSP, DB)       ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
# ── config.py ────────────────────────────────────────────────────────
try:
    from config import Config as RFConfig, DemodConfig, DspConfig
    _RF_CONFIG_OK = True
except ImportError:
    _RF_CONFIG_OK = False
...
try:
    ...
    _RF_LOGGER_OK = False
    def _rf_get_logger(name): return logging.getLogger(name)       # type: ignore
    def _rf_setup_logger(**kw): return logging.getLogger("rfscanner")  # type: ignore
...
class RFModuleIntegrado:
    ...
    def _inicializar_subsistemas(self):
        ...
        if _RF_LOGGER_OK:
            try:
                _rf_setup_logger(level="INFO", log_file="data/logs/rfscanner.log")
            except Exception:
                pass
        ...
        if _RFSCANNER_OK and MotorDSP and _NP_OK:
            try:
                self._dsp_basico = MotorDSP(fft_size=self.fft_size, ventana="blackman")
            except Exception as e:
    ...
    def _conectar_hardware(self):
        if _RFSCANNER_OK and RFScanner:
            try:
                ...
                if self._scanner.sdr is not None:
                    ...
                    self._print(f"[green][+] RF Hardware real — {self.hw_nombre}[/green]")
                    return
                self._scanner = None
            except Exception as e:
        ...
        if _MOCK_OK and MockSDRManager and _NP_OK:
            try:
                self._mock = MockSDRManager(sample_rate=self.sample_rate, noise_floor_dbm=-100.0)
                if SyntheticSignal:
                    self._mock.add_signal(SyntheticSignal(freq_offset=0,    power_dbm=-60, mode="nfm"))
                    self._mock.add_signal(SyntheticSignal(freq_offset=50e3, power_dbm=-70, mode="wfm"))
                    self._mock.add_signal(SyntheticSignal(freq_offset=-30e3,power_dbm=-75, mode="am"))
        ...
        self.hw_disponible = False
        self._print("[red][!] No se pudo inicializar ningún backend RF.\n    Instala: pip install numpy pyrtlsdr rich[/red]")
    ...
    def _calcular_psd_y_picos(self, muestras: "np.ndarray", freq_hz: float):
        ...
        if self._dsp_basico is not None:
            try:
                freqs, psd = self._dsp_basico.calcular_psd(muestras, self.sample_rate)
                picos      = self._dsp_basico.detectar_picos(freqs, psd, freq_hz, self.sample_rate)
                for p in picos:
                    bw = p.get("bw_khz", 0)
    ...
    def _exportar_csv(self, picos: list, freq_mhz: float):
        ...
        try:
            campos = ["freq_mhz", "potencia", "snr_db", "bw_khz", "piso_dbm", "mod_hint", "banda", "timestamp"]
            with open(fn, "w", newline="", encoding="utf-8") as f:
                w = csv.DictWriter(f, fieldnames=campos)
                w.writeheader()
                for p in picos:
                    row = {k: p.get(k, "") for k in campos}
                    row["banda"] = p["banda"]["nombre"] if p.get("banda") else "—"
                    w.writerow(row)
    ...
    def _exportar_csv_barrido(self, resultados: list, freq_ini: float, freq_fin: float):
        ...
        try:
            with open(fn, "w", newline="", encoding="utf-8") as f:
                w = csv.DictWriter(f, fieldnames=["freq_mhz", "pot_max", "piso", "snr", "banda"])
                w.writeheader()
                for r in resultados:
                    w.writerow({
    ...
    def _render_tabla_picos(self, picos: list) -> Panel:
        if self._render is not None:
            try:
                ...
                pass
        tb = Table(box=box.SIMPLE_HEAD, header_style="bold green", show_edge=False, expand=True)
        ...
        for p in picos:
            ...
            potencia_txt = Text(
                f"{p['potencia']:.1f} dBm",
                style="bold red" if p["potencia"] > -50 else "yellow" if p["potencia"] > -70 else "green"
            )
    ...
    def _render_resumen(self, freq_mhz: float, picos: list, duracion: float, iteraciones: int) -> Panel:
        ...
        g.add_row("DSP Engine",         "Avanzado (CFAR)" if self._dsp_avanzado else "Básico (MotorDSP)")
        g.add_row("Base de datos",      "SQLite activa"   if self._db else "Sin persistencia")
    ...
    def escanear_frecuencia(self, freq_mhz: float, duracion: int = 10):
        ...
        if banda:
            ...
            self._print(f"[bold green][RF] Sintonizando {freq_mhz:.4f} MHz — [{col}]{banda['nombre']}[/{col}]  [dim]{banda.get('desc', '')}[/dim][/bold green]")
        else:
            self._print(f"[bold green][RF] Sintonizando {freq_mhz:.4f} MHz — Banda no clasificada[/bold green]")

        self._print(f"[dim]  HW: {self.hw_nombre}  |  BW: {self.sample_rate/1e6:.3f} MHz  |  FFT: {self.fft_size} pts  |  Duración: {duracion}s  |  Ctrl+C para detener[/dim]\n")
        time.sleep(0.4)

        escaneo_id: Optional[int] = None
        if self._db:
            try:
                escaneo_id = self._db.iniciar_escaneo(freq_mhz, self.hw_nombre, int(self.sample_rate), self.fft_size)
            except Exception:
                pass
        ...
        try:
            while time.time() - inicio < duracion:
                ...
                for _ in range(self.PROMEDIO_N):
                    ...
                    if self._dsp_basico:
                        try:
                            _, psd = self._dsp_basico.calcular_psd(muestras, self.sample_rate)
                ...
                freqs, psd, picos = self._calcular_psd_y_picos(muestras_last, freq_hz)
                ...
                if freqs is not None and psd is not None:
                    self.console.print(self._render_espectro(freqs, psd, freq_mhz, picos))
        ...
        self.console.print()
        self.console.print(self._render_resumen(freq_mhz, todos_picos, duracion_real, iteracion))
        ...
        if self.log_s:
            self.log_s.info(f"Escaneo RF {freq_mhz:.3f} MHz: {len(todos_picos)} señales en {duracion_real:.0f}s", "RFScanner")
    ...
    def barrido_espectro(self, freq_ini_mhz: float, freq_fin_mhz: float, paso_mhz: float = 1.0):
        ...
        try:
            for i, freq in enumerate(freqs):
                ...
                piso    = self._dsp_basico.estimar_piso_ruido(psd) if self._dsp_basico else float(np.median(psd))
                ...
                barra = "█" * pct + "─" * (50 - pct)
                print(f"\r  [{barra}] {freq:.2f} MHz  {pot_max:.1f}dBm  SNR:{snr:.1f}dB  {banda['nombre'] if banda else '—':<20}", end="")
        ...
        if resultados:
            ...
            if self._db:
                try:
                    self._db.insertar_barrido(freq_ini_mhz, freq_fin_mhz, paso_mhz, self.hw_nombre, resultados)
                except Exception:
                    pass
    ...
    def escaneo_bandas_conocidas(self):
        ...
        bandas_a_escanear = (
            [(fmin, fmax, nombre, tipo, desc, "white") for fmin, fmax, nombre, tipo, desc, _ in BANDAS_RF]
        ...
        for fmin, fmax, nombre, tipo, desc, color in bandas_a_escanear:
            ...
            print(f"\r  {nombre:<25} {freq:>8.2f} MHz  {pot_max:>6.1f} dBm  SNR: {snr:>5.1f} dB", end="")
    ...
    def estado(self):
        ...
        g.add_column(style="white")
        g.add_row("Hardware",             self.hw_nombre)
        g.add_row("Disponible",           "[green]Sí[/green]" if self.hw_disponible else "[red]No[/red]")
        g.add_row("Sample rate",          f"{self.sample_rate/1e6:.3f} MHz")
        g.add_row("Ganancia",             f"{self.gain} dB")
        g.add_row("FFT size",             str(self.fft_size))
        g.add_row("DSP Engine",           "[green]Avanzado (CFAR)[/green]" if self._dsp_avanzado else "[yellow]Básico[/yellow]")
        g.add_row("Demodulador",          "[green]Activo[/green]" if self._demod else "[dim]No disponible[/dim]")
        g.add_row("Base de datos SQLite", "[green]Activa[/green]" if self._db else "[dim]No disponible[/dim]")
        ...
        g.add_row("Bandas tácticas",      str(len(tactical_bands())) if _BANDS_OK else "—")
        g.add_row("numpy",                "[green]OK[/green]" if _NP_OK else "[red]NO[/red]")
        g.add_row("Config TOML",          "[green]OK[/green]" if _RF_CONFIG_OK else "[yellow]Sin TOML (defaults)[/yellow]")
        self.console.print(Panel(g, title="[bold green]ESTADO RF SCANNER[/bold green]", border_style="green"))
    ...
    def db_consultar(self, freq_min=None, freq_max=None, snr_min=None, horas=None):
        if not self._db:
            ...
            return
        try:
            resultados = self._db.consultar_senales(freq_min=freq_min, freq_max=freq_max, snr_min=snr_min, horas=horas)
            if not resultados:
            ...
            self.console.print(Panel(tb, title=f"[bold green]DB RF — {len(resultados)} señales[/bold green]", border_style="green"))
        except Exception as e:
    ...
    def db_estadisticas(self):
        ...
        try:
            ...
            self.console.print(Panel(g, title="[bold green]ESTADÍSTICAS DB RF[/bold green]", border_style="green"))
        except Exception as e:
    ...
    def menu(self):
        ...
        if opt == "1":
            ...
            try:
                self.escanear_frecuencia(float(freq_s), int(dur_s) if dur_s else 10)
            except ValueError:
            ...
            try:
                self.barrido_espectro(float(ini_s), float(fin_s), float(paso_s) if paso_s else 1.0)
            except ValueError:
            ...
            gan_s = self.console.input("[bold cyan][?] Ganancia dB (0-49, 'auto'): [/bold cyan]").strip()
            self.configurar_ganancia("auto" if gan_s.lower() == "auto" else float(gan_s) if gan_s else 40)
        elif opt == "5":
            self.console.print(self._render_tabla_picos(self._senales_sesion[-50:])) if self._senales_sesion else self._print("[dim]Sin señales en esta sesión.[/dim]")
        elif opt == "6":
            self.estado()
        elif opt == "7":
            freq_s = self.console.input("[bold cyan][?] Frecuencia mínima MHz (Enter=todas): [/bold cyan]").strip()
            snr_s  = self.console.input("[bold cyan][?] SNR mínimo dB [0]: [/bold cyan]").strip()
            hs_s   = self.console.input("[bold cyan][?] Últimas N horas (Enter=todas): [/bold cyan]").strip()
            ...
            if not bandas:
                ...
                return
            tb = Table(box=box.SIMPLE_HEAD, header_style="bold red", show_edge=False, expand=True)
            ...
            self.console.print(Panel(tb, title="[bold red]BANDAS TÁCTICAS[/bold red]", border_style="red"))
