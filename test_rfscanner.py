...
@pytest.fixture
def dsp_cfg():
    from rf_config import DspConfig
    return DspConfig(
        fft_size=2048,
        window="blackman",
        snr_threshold=8.0,
...
def mock_sdr(sample_rate):
    from rf_mock import MockSDRManager
...
def signal_db(storage_cfg):
    from rf_storage import SignalDB
...
class TestConfig:
    def test_default_loads(self):
        from rf_config import load_config
        cfg = load_config()
    ...
    def test_save_and_reload(self, tmp_dir):
        from rf_config import load_config, save_config
        cfg = load_config()
...
class TestBands:
    def test_fm_radio_identified(self):
        from bands import identify_band
    ...
    def test_bands_in_range(self):
        from bands import bands_in_range
    ...
    def test_tactical_bands_not_empty(self):
        from bands import tactical_bands
        tac = tactical_bands()
        assert len(tac) > 0
...
class TestMockSDR:
    ...
    def test_capture_with_signals(self, sample_rate):
        from rf_mock import MockSDRManager, SyntheticSignal
    ...
    def test_from_file(self, tmp_dir, sample_rate):
        from rf_mock import generate_fixture, MockSDRManager, SyntheticSignal
...
class TestDemodulator:
    ...
    def test_nfm_output_shape(self, demod, sample_rate):
        from rf_mock import generate_iq, SyntheticSignal
...
class TestCSVExporter:
    def test_export_signals(self, storage_cfg, tmp_dir):
        from rf_storage import CSVExporter
        from dsp import Signal
...
class TestSigMF:
    def test_write_sigmf(self, storage_cfg):
        from rf_storage import SigMFWriter
...
class TestCapturePipeline:
    def test_pipeline_produces_samples(self, mock_sdr, dsp_cfg):
        ...
        from rf_mock import SyntheticSignal
...
class TestRealHardware:
    def test_connect(self, hardware_required):
        from hardware import SDRManager
        from rf_config import HardwareConfig
    ...
    def test_permissions(self, hardware_required):
        from hardware import check_permissions
