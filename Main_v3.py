...
from bootscreen import (
    ...
    MODULOS_BOOT, ESTILOS_LOG, ANUBIS_ART,
)
...
class ApexSentinel:
    ...
    def mostrar_dashboard_exito(self, ip: str, servicio: str, credencial: str):
        ...
        self.console.print(Panel(
            tabla, title="[bold green]MISSION ACCOMPLISHED[/bold green]", border_style="bright_green", expand=False))
        self.console.print(
        ...
        if self.gp:
            self.gp.registrar_hallazgo(
                "CRITICO", f"Credenciales obtenidas en {ip}:{servicio}", f"Credenciales válidas: {credencial}", "Cambiar credenciales inmediatamente.")
    ...
    def _cmd_status(self):
        proy = (
            self.gp.proyecto_activo.nombre if self.gp and self.gp.proyecto_activo else "Ninguno")
        self.console.print(Panel(
    ...
    def _cmd_scan(self):
        ...
        try:
            resultado = self._srp(self._Ether(
                dst="ff:ff:ff:ff:ff:ff") / self._ARP(pdst=rango), timeout=3, verbose=False)[0]
            tabla = Table(header_style="bold cyan",
                          box=box.SIMPLE_HEAD, show_edge=False)
            ...
            self.console.print(tabla)
            if self.gp:
                self.gp.registrar_evidencia("arp_scan", f"Scan ARP en {rango}: {len(hosts)} hosts", {
    ...
    def _cmd_portscan(self):
        ...
        puertos = {21: "FTP", 22: "SSH", 23: "Telnet", 25: "SMTP", 80: "HTTP",
                   443: "HTTPS", 445: "SMB", 3306: "MySQL", 5432: "PostgreSQL", 8080: "HTTP-Alt"}
    ...
    def _cmd_audit(self):
        if self._modulo_ok("hydra") and self._modulo_ok("dict_manager"):
            self.console.print(
    ...
    def _cmd_vulnscan(self):
        if self._modulo_ok("audit_engine"):
        ...
        if self._modulo_ok("bt"):
            ...
            if opt == "1":
                ...
                mac_vic = Validador.pedir(
                    self.console, "MAC Víctima", Validador.es_mac, "MAC inválida.")
    ...
    def _cmd_view(self):
        if self._modulo_ok("reader"):
            ruta_base = "./data/evidence/mobile/Deep_Extraction/"
            opcion = self.console.input(
                "[bold cyan] [1] Leer WhatsApp  [2] Leer Chrome: [/bold cyan]").strip()
            if opcion == "1":
    ...
    def _cmd_ducky(self):
        if self._modulo_ok("ducky"):
            self.ducky.ejecutar_payload()

    def _cmd_stealth(self):
        if self._modulo_ok("stealth"):
    ...
    def _cmd_proyecto(self, args: list):
        if self._modulo_ok("gp"):
            sub = args[0] if args else ""
            acciones = {"nuevo": self.gp.crear_proyecto, "cargar": self.gp.cargar_proyecto,
                        "lista": self.gp.listar_proyectos, "estado": self.gp.mostrar_resumen, "cerrar": self.gp.cerrar_proyecto}
            accion = acciones.get(sub)
            if accion:
                accion()
            else:
                self.console.print(
                    "[dim]Subcomandos: nuevo | cargar | lista | estado | cerrar[/dim]")

    def _cmd_reporte(self, args: list):
        if self._modulo_ok("motor_rep"):
            sub = args[0] if args else ""
            if sub == "resumen":
    ...
    def _cmd_osint(self):
        if self._modulo_ok("osint"):
            self.osint.menu()

    def _cmd_cve(self):
        if self._modulo_ok("cve"):
            self.cve.busqueda_libre()

    def _cmd_jobs(self, args: list):
        if self._modulo_ok("cola"):
            sub = args[0] if args else ""
    ...
    def _despachar(self, entrada: str) -> bool:
        ...
        # Comandos complejos con sub-lógica
        if cmd == "proyecto":
        ...
        # --- DICCIONARIO DE AYUDA (FIJO PARA EVITAR NameError) ---
        comandos_help = {
            "help":     "Muestra este menú de ayuda táctico",
            "status":   "Estado de los subsistemas y red",
            "phishing": "Inicia el servidor de phishing zphisher",
            "scan":     "Escaneo de red local (ARP/IP)",
            "wifi":     "Ataques de deauth y beacon spam",
            "clear":    "Refresca el banner del sistema",
            "panic":    "Cierre forzado y limpieza de huellas"
        }

        # --- TABLA DE DESPACHO ---
        # USAMOS ARGUMENTOS POR NOMBRE (KEYWORD ARGUMENTS) PARA EVITAR ERRORES DE ORDEN
        tabla = {
            "help": lambda: mostrar_ayuda(console=self.console, version=self.version, ayuda=comandos_help),
            "?": lambda: mostrar_ayuda(console=self.console, version=self.version, ayuda=comandos_help),
        ...
        if cmd in tabla:
            try:
                ...
                self.log.error(f"Error al ejecutar {cmd}: {e}", "Despachador")
                return True
    ...
    def ejecutar(self):
        ...
        while True:
            try:
                plab = f"\\[{self.gp.proyecto_activo.nombre}]" if self.gp and self.gp.proyecto_activo else ""
                prompt_str = f"[bold green]AnubisOS[/bold green][dim white]@[/dim white][bold cyan]Sentinel[/bold cyan][dim]{plab}[/dim][bold white]~#[/bold white] "
                ...
                if not self._despachar(entrada):
                    self.console.print(
                        f"[yellow][?] Comando '{entrada}' no reconocido. Escribe 'help'.[/yellow]")
